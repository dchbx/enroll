...
class BenefitApplicationMigration < Mongoid::Migration
  ...
  def self.convert_plan_year_to_benefit_application(benefit_sponsorship, plan_year, _csv)
    ...
    if plan_year.start_on == plan_year.end_on
      benefit_application.write_attribute(:effective_period, (plan_year.start_on..plan_year.end_on))
    end  # benefit_application.effective_period setter method setting value to nil if plan_year.start_on == plan_year.end_on for those cases uses below
    ...
    if predecessor_application.present?
      benefit_application.predecessor_id = if predecessor_application.count < 2
                                             predecessor_application.first.id
                                           elsif predecessor_application.where(:"effective_period.max" => Date.new(2018,7,31)).count == 2  # exception case for 8/1 conversion
                                             predecessor_application.where(aasm_state: :imported).first.id
                                           else
                                             predecessor_application.first.id
                                           end
    end
    ...
    self.update_sponsor_catalog_product_package(@benefit_sponsor_catalog, plan_year) unless self.new_benefit_sponsor_catalog_product_valid(catalog_product_hios_id, plan_year_plan_hios_ids)
    ...
    benefit_application.aasm_state = :imported if plan_year.is_conversion
  ...
  def self.get_benefit_sponsorship_effective_on(old_org)
    ...
    plan_years.first.start_on if plan_years.present?
  end
  ...
  def self.set_benefit_sponsorship_state(old_org, benefit_sponsorship)
    ...
    if benefit_sponsorship.source_kind.to_s == "self_serve"
      ...
      benefit_sponsorship.aasm_state = if old_org.employer_profile.published_plan_year.present? && old_org.employer_profile.published_plan_year.enrolling?
                                         :applicant
                                       else
                                         benefit_sponsorship.send(:employer_profile_to_benefit_sponsor_states_map)[old_org.employer_profile.aasm_state.to_sym]
                                       end
    end
  end
    ...
    @benefit_package_map.each do |benefit_group, benefit_package|
      benefit_group.census_employees.unscoped.each do |census_employee|
        if census_employee.benefit_sponsorship_id.blank?
          if census_employee.employee_role && census_employee.employee_role.benefit_sponsors_employer_profile_id.blank?
            census_employee.employee_role.update_attributes(benefit_sponsors_employer_profile_id: benefit_sponsorship.organization.employer_profile.id)
          end
        ...
        census_employee.benefit_group_assignments.unscoped.each do |benefit_group_assignment|
          benefit_group_assignment.benefit_package_id = benefit_package.id if benefit_group_assignment.benefit_group_id.to_s == benefit_group.id.to_s
        end
