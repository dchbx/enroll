namespace :shop do
  ...
  task :products_seed => :environment do
    ...
    years.each do |year|
      ...
      rating_area_id = ::BenefitMarkets::Locations::RatingArea.where(active_year: year).first.id
      ...
      data.each do |product_hash|
        ...
        product_hash[:service_area_id] = service_areas.where(issuer_profile_id: product_hash[:issuer_profile_id])
