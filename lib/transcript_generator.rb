...
class TranscriptGenerator
  ...
  def execute
    ...
    Dir.glob("#{Rails.root}/sample_xmls/*.xml").each do |file_path|
      ...
      my_logger.info("failed to process #{file_path}---#{e}")
  ...
  def display_enrollment_transcripts
    ...
    CSV.open("#{@market}_enrollment_change_sets_#{Time.now.strftime('%m_%d_%Y_%H_%M')}.csv", "w") do |csv|
      csv << if @market == 'individual'
               ['Enrollment HBX ID', 'Subscriber HBX ID','SSN', 'Last Name', 'First Name', 'HIOS_ID:PlanName', 'Other Effective On','Effective On', 'AASM State', 'Terminated On', 'Action', 'Section:Attribute', 'Value']
             else
               ['Enrollment HBX ID', 'Subscriber HBX ID','SSN', 'Last Name', 'First Name', 'HIOS_ID:PlanName', 'Other Effective On','Effective On', 'AASM State', 'Terminated On', 'Employer FEIN', 'Employer Legalname', 'Action', 'Section:Attribute', 'Value']
             end
      ...
      #   file_path = "#{Rails.root}/LatestXmlFiles/ivl_final/#{row["Enrollment HBX ID"].strip}.xml"
        ...
        if rows.empty?
          csv << if @market == 'individual'
                   (first_row[0..9] + ['match', 'match:enrollment'] + [''] + enrollment_transcript.updates['match:enrollment'].to_a)
                 else
                   (first_row[0..11] + ['match', 'match:enrollment'])
                 end
