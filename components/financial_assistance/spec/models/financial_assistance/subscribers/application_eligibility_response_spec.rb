...
RSpec.describe ::FinancialAssistance::Subscribers::ApplicationEligibilityResponse do
  before :each do
  ...
  it 'should subscribe to the correct event' do
    expect(FinancialAssistance::Subscribers::ApplicationEligibilityResponse.subscription_details).to eq ['acapi.info.events.assistance_application.application_processed']
  end
  ...
  describe 'given a valid payload with a medicad member and get status 203' do
    ...
    context 'update application as determined with status code' do
      ...
      context 'with primary person, family, body, a valid application, return status and not connected to application to person' do
        ...
        let(:parser) { Parsers::Xml::Cv::HavenVerifiedFamilyParser.new.parse(xml) }
        let(:primary_person) { FactoryBot.create(:person) }
        let!(:primary_person_family)  { FactoryBot.create(:family, :with_primary_family_member, person: primary_person) }
        let(:person) { FactoryBot.create(:person) }
        let!(:family)  { FactoryBot.create(:family, :with_primary_family_member, person: person) }
        let!(:application) do
      ...
      #   let!(:family)  { FactoryBot.create(:family, :with_primary_family_member, person: person) }
      #   let!(:application) {
      #     FactoryBot.create(:application, hbx_id: '#{parser.fin_app_id}', family: family, aasm_state: 'submitted')
      #   }
      #   let(:tax_household) { FactoryBot.create(:tax_household, application: application) }
      #
