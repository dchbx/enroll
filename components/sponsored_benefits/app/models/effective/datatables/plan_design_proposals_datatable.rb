  ...
  module Datatables
    class PlanDesignProposalsDatatable < ::Effective::MongoidDatatable
      datatable do
        ...
        table_column :title, :label => 'Quote Name', :proc => proc { |row|
                                                                if row.published?
        ...
        table_column :effective_date, :label => 'Effective Date', :proc => proc { |row| proposal_sponsorship(row).initial_enrollment_period.begin.strftime("%Y - %m - %d") }, :sortable => true, :filter => false
        table_column :claim_code, :label => 'Claim Code', :proc => proc { |row| row.claim_code || 'Not Published' }, :sortable => false, :filter => false
        table_column :employees, :label => 'Employees', :proc => proc { |row| proposal_sponsorship(row).census_employees.count }, :sortable => false, :filter => false
        table_column :families, :label => "Families", :proc => proc { |row| proposal_sponsorship(row).census_employees.where({ "census_dependents.0" => { "$exists" => true } }).count }, :sortable => false, :filter => false
        table_column :plan_option_kind, :label => "Plan Type", :proc => proc { |row|
        ...
        table_column :reference_plan, :label => "Reference Plan", :proc => proc { |row|
          if has_assigned_benefit_group?(row)
        ...
        table_column :state, :label => 'State', :proc => proc { |row| row.aasm_state.capitalize }, :sortable => false, :filter => false
        ...
        table_column :actions, :width => '50px', :proc => proc { |row|
          ...
          render partial: 'datatables/shared/dropdown', locals: {dropdowns: dropdown, row_actions_id: "quotes_actions_#{row.id}"}, formats: :html
      ...
      def collection
        @quotes = Queries::PlanDesignProposalsQuery.new(attributes) unless (defined? @quotes) && @quotes.present?
        @quotes
      end
