  ...
  module Datatables
    class PlanDesignEmployeeDatatable < ::Effective::MongoidDatatable
      ...
      datatable do
        ...
        table_column :census_dependents, :label => 'No. of Dependents', :proc => proc { |row|
        ...
        table_column :actions, label: "", :width => '50px', :proc => proc { |row|
          ...
          render partial: 'datatables/shared/dropdown', locals: {dropdowns: dropdown, row_actions_id: "plan_design_employee_#{row.id}"}, formats: :html
        }, :filter => false, :sortable => false
      end

      def collection
        @employees = Queries::PlanDesignEmployeeQuery.new(attributes) unless (defined? @employees) && @employees.present? #memoize the wrapper class to persist @search_string
        @employees
      end
