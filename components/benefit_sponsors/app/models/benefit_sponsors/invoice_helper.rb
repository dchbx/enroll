    ...
    def build_pdf
      ...
      enrollment_summary.each do |name, summary|
        ...
        summary["enrollments"].each do |enrollment|
          ...
          carrier_plan_services_data << [subscriber.last_name.to_s, subscriber.first_name.to_s,enrollment.humanized_members_summary.to_s, DateTime.now.next_month.strftime('%m/%Y').to_s,"$#{currency_format(enrollment.total_employer_contribution)}","$#{currency_format(enrollment.total_employee_cost)}","$#{currency_format(enrollment.total_premium)}"]
        end
