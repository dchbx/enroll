...
module BenefitSponsors
  RSpec.describe ContributionCalculators::TieredPercentWithCapContributionCalculator, :dbclean => :after_each do
    let(:contribution_calculator) do
      ::BenefitSponsors::ContributionCalculators::TieredPercentWithCapContributionCalculator.new
    end
    ...
    - with a 'family' group" do
      ...
      let(:employee_only_contribution_unit) do
        instance_double(
          ::BenefitMarkets::ContributionModels::PercentWithCapContributionUnit,
          name: "employee_only",
      ...
      describe "with a pricing determination and one family that has:
        ...
        - a spouse
        - a child
        - and comes in under the cap for percentage contributions
      " do

        let(:contribution_cap) { 90.00 }
        ...
        let(:sponsor_contribution) do
          instance_double(
            ::BenefitSponsors::SponsoredBenefits::FixedPercentWithCapSponsorContribution,
      ...
      describe "with a pricing determination and one family that has:
        ...
        - a spouse
        - a child
        - and exceeds the cap for percentage contributions
      " do

        let(:contribution_cap) { 50.00 }
        ...
        it "calculates the total contribution" do
          ...
          expect(calculation_result.group_enrollment.sponsor_contribution_total).to eq(contribution_cap)
        end
