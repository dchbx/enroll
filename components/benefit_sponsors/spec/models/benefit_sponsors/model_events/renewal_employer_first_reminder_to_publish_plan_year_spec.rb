...
RSpec.describe 'BenefitSponsors::ModelEvents::RenewalEmployerReminderToPublishPlanYearNotification', dbclean: :around_each do
  ...
  let(:model_event) { "renewal_employer_first_reminder_to_publish_plan_year" }
  ...
  let!(:date_mock_object) { Date.new(renewal_effective_date.year, renewal_effective_date.prev_month.month, (Settings.aca.shop_market.renewal_application.application_submission_soft_deadline - 2)) }
    ...
    it "should trigger model event" do
      model_instance.class.observer_peers.keys.select{ |ob| ob.is_a? BenefitSponsors::Observers::NoticeObserver }.each do |observer|
        expect(observer).to receive(:process_application_events) do |_instance, model_event|
          ...
          expect(model_event).to have_attributes(:event_key => :renewal_employer_first_reminder_to_publish_plan_year, :klass_instance => model_instance, :options => {})
        end

        expect(observer).to receive(:process_application_events).twice
      end
  ...
  describe "NoticeTrigger" do
    context "2 days prior to soft dead line" do
      ...
      let(:model_event) { BenefitSponsors::ModelEvents::ModelEvent.new(:renewal_employer_first_reminder_to_publish_plan_year, model_instance, {}) }

      it "should trigger notice event" do
        expect(subject.notifier).to receive(:notify) do |event_name, payload|
          expect(event_name).to eq "acapi.info.events.employer.renewal_employer_first_reminder_to_publish_plan_year"
