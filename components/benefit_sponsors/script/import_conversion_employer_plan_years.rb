  ...
  class ConversionEmployerPlanYears
    ...
    def import_employer(in_file)
      ...
      importer = if BenefitSponsors::Site.by_site_key(:cca).present?
                   BenefitSponsors::Importers::Mhc::ConversionEmployerPlanYearSet.new(in_file, result_file, config)
                 else
                   ::Importers::ConversionEmployerPlanYearSet.new(in_file, result_file, config["conversions"]["plan_year_date"])
                 end
