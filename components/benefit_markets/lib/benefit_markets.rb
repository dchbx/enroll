...
module BenefitMarkets
  BENEFIT_MARKET_KINDS    = [:aca_shop, :aca_individual, :fehb, :medicaid, :medicare].freeze
  PRODUCT_KINDS           = [:health, :dental, :term_life, :short_term_disability, :long_term_disability].freeze
  PROBATION_PERIOD_KINDS  = [:first_of_month_before_15th, :date_of_hire, :first_of_month, :first_of_month_following, :first_of_month_after_30_days, :first_of_month_after_60_days].freeze
  ...
  APPLICATION_INTERVAL_KINDS  = [:monthly, :annual, :annual_with_midyear_initial].freeze


  CONTACT_METHOD_KINDS        = [:paper_and_electronic, :paper_only, :electronic_only].freeze
  ...
  def self.strip_leading_numbers(string_value)
    string_value = string_value.slice!(1, string_value.length - 1) while string_value.chr.numeric?
    string_value
  end
