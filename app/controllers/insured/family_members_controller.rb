class Insured::FamilyMembersController < ApplicationController
  ...
  def update
    ...
    @valid_vlp = if params[:person][:is_applying_coverage].present?
                    params[:person][:is_applying_coverage] == 'false' ? false : update_vlp_documents?(@person.consumer_role, 'person')
                  else
                    update_vlp_documents?(@person.consumer_role, 'person')
                  end
    if @dependent.update_attributes(params.require(:dependent)) && @valid_vlp.blank?
      consumer_role = @dependent.family_member.try(:person).try(:consumer_role)
      if consumer_role.present?
        ...
        consumer_role.update_attribute(is_applying_coverage: params[:dependent][:is_applying_coverage]) if params[:dependent][:is_applying_coverage].present?
      end
      respond_to do |format|
        format.html { render 'show' }
        format.js { render 'show' }
