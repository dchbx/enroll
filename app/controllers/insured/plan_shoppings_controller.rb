...
module Insured
  ...
  # Waives against an existing enrollment
    ...
    def extract_from_shop_products
      case @hbx_enrollment.coverage_kind
      when 'health'
  ...
  # no dental as of now
    def sort_member_groups(products)
      products.select { |prod| prod.group_enrollment.product.id.to_s == @enrolled_hbx_enrollment_plan_ids.first.to_s } + products.reject do |prod|
