...
class ApplicationController < ActionController::Base
  ...
  def confirm_last_portal(request, resource)
    ...
    if last_portal_visited
      ...
      redirect_path = if last_portal_host
                        current_host == last_portal_host ? last_portal_visited : local_path
                      else
                        last_portal_visited
                      end
    else
  ...
  def set_current_user
    ...
    SessionIdHistory.create(session_id: session_id, session_user_id: current_user.try(:id), sign_in_outcome: "Successful", ip_address: request.remote_ip) unless SessionIdHistory.where(session_id: session_id).present?
  end
  ...
  def hbx_staff_and_consumer_role(role)
    ...
    if role.present?
      hbx_staff.present? && role.instance_of?(ConsumerRole)
    else
