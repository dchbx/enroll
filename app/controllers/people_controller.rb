class PeopleController < ApplicationController
  ...
  def update
    ...
    if @person.is_consumer_role_active? && request.referer.include?("insured/families/personal")
      # No need to update vlp documents if not applying for coverage
      @valid_vlp = person_params[:is_applying_coverage] == 'false' ? false : update_vlp_documents?(@person.consumer_role, 'person')
    ...
    respond_to do |format|
      if @valid_vlp.blank? && @person.update_attributes(person_params.except(:is_applying_coverage))
        ...
        if person_params[:is_applying_coverage] && @person.consumer_role.present?
          @person.consumer_role.update_attribute(is_applying_coverage: params[:person][:is_applying_coverage])
        end
