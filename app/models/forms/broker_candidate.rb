...
module Forms
  class BrokerCandidate < ::Forms::PersonSignup
    ...
    attr_accessor :broker_agency_id, :broker_applicant_type, :market_kind, :languages_spoken, :working_hours, :accept_new_clients, :addresses

    validate :validate_broker_agency
    validate :validate_duplicate_npn, :if => proc {|p| p.broker_applicant_type != 'staff'}

    validates :npn,
              length: { maximum: 10, message: "%{value} is not a valid NPN" },
              format: { with: /\A[1-9][0-9]+\z/, message: "%{value} is not a valid NPN" },
              numericality: true,
              :if => proc {|p| p.broker_applicant_type != 'staff'}
    ...
    def validate_duplicate_npn
      errors.add(:base, "NPN has already been claimed by another broker. Please contact HBX.") if Person.where("broker_role.npn" => npn).any?
    end

    def broker_role?
      self.broker_applicant_type != 'staff'
    end
