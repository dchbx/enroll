...
module Forms
  class BulkActionsForAdmin
    ...
    attr_reader :result, :row, :family_id, :params, :config
    ...
    def cancel_enrollments
      ...
      @params.each do |key, _value|
        next unless key.to_s[/cancel_hbx_.*/]
        hbx = HbxEnrollment.find(params[key.to_s])
        begin
          hbx.cancel_coverage! if hbx.may_cancel_coverage?
          @result[:success] << hbx
          cancelled_enrollments_transmission_info[hbx.id] = params.key?("transmit_hbx_#{hbx.id}") ? true : false
    ...
    def terminate_enrollments
      ...
      @params.each do |key, value|
        next unless key.to_s[/terminate_hbx_.*/]
        hbx = HbxEnrollment.find(params[key.to_s])
        begin
          ...
          @result[:success] << hbx
          terminated_enrollments_transmission_info[hbx.id] = params.key?("transmit_hbx_#{hbx.id}") ? true : false
        rescue StandardError
          @result[:failure] << hbx
        end
      end
    ...
    def transition_family_members
      ...
      params.each do |key, value|
        next unless key.to_s[/transition_user_.*/]
...
#transmit_flag = true/false based on wheather the user elected to transmit.
    def handle_edi_transmissions(hbx_id, transmit_flag)
