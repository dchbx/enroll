...
module Forms
  class EmployerProfile < ::Forms::OrganizationSignup
    ...
    attr_accessor :email, :area_code, :number, :extension
    ...
    def check_existing_organization
      ...
      claimed = true if existing_org.present? && existing_org.employer_profile.present? && Person.where({"employer_staff_roles.employer_profile_id" => existing_org.employer_profile._id}).any?
    ...
    def create_employer_staff_role(current_user, employer_profile, existing_company)
      ...
      if employer_ids.include? employer_profile.id
        ...
        pending = existing_company && Person.staff_for_employer(employer_profile).detect(&:user_id)
    ...
    def save(current_user, _employer_profile_id)
    ...
    def update_organization(org)
      unless org.employer_profile.present?
