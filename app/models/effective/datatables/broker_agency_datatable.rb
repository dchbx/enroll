  ...
  module Datatables
    class BrokerAgencyDatatable < Effective::MongoidDatatable
      datatable do
        ...
        table_column :dba, :proc => proc { |row|
          row.dba
        }, :sortable => false, :filter => false
        table_column :fein, :label => 'FEIN', :proc => proc { |row| row.fein }, :sortable => false, :filter => false

        table_column :entity_kind, :proc => proc { |row| row.entity_kind.to_s.titleize }, :sortable => false, :filter => false
        table_column :market_kind, :proc => proc { |row| row.broker_agency_profile.market_kind.to_s.titleize }, :sortable => false, :filter => false
