  ...
  module Datatables
    class EmployerDatatable < Effective::MongoidDatatable
      ...
      datatable do
        ...
        table_column :fein, :label => 'FEIN', :proc => proc { |row| row.fein }, :sortable => false, :filter => false
        table_column :hbx_id, :label => 'HBX ID', :proc => proc { |row| row.hbx_id }, :sortable => false, :filter => false
#        table_column :eligibility, :proc => Proc.new { |row| eligibility_criteria(@employer_profile) }, :filter => false
        ...
        table_column :general_agency, :proc => proc { |_row|
        ...
        table_column :conversion, :proc => proc { |_row| boolean_to_glyph(@employer_profile.is_conversion?)}, :filter => {include_blank: false, :as => :select, :collection => ['All','Yes', 'No'], :selected => 'All'}

        table_column :plan_year_state, :proc => proc { |_row|
        ...
        table_column :effective_date, :proc => proc { |_row|
                                                 @latest_plan_year.try(:start_on)
                                               }, :filter => false, :sortable => true
        table_column :invoiced?, :proc => proc { |row| boolean_to_glyph(row.current_month_invoice.present?)}, :filter => false
