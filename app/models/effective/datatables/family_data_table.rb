      ...
      datatable do
        ...
        table_column :actions, :width => '50px', :proc => proc { |row|
          dropdown = [
           ...
           ['Change Enrollment End Date', view_enrollment_to_update_end_date_exchanges_hbx_profiles_path(family: row.id, person_id: row.primary_applicant.person.id, family_actions_id: "family_actions_#{row.id}"), update_terminated_enrollment_type(row, pundit_allow(Family, :can_update_ssn?))],
           ['Reinstate', view_terminated_hbx_enrollments_exchanges_hbx_profiles_path(family: row.id, person_id: row.primary_applicant.person.id, family_actions_id: "family_actions_#{row.id}"), reinstate_enrollment_type(row, pundit_allow(Family, :can_update_ssn?))],
           ...
           ['Send Secure Message', new_insured_inbox_path(id: row.primary_applicant.person.id, profile_id: current_user.person.hbx_staff_role.hbx_profile.id, to: row.primary_applicant.person.last_name + ', ' + row.primary_applicant.person.first_name, family_actions_id: "family_actions_#{row.id}"), secure_message_link_type(row, current_user)],
           ['View Username and Email', get_user_info_exchanges_hbx_profiles_path(person_id: row.primary_applicant.person.id, family_actions_id: "family_actions_#{row.id}"), individual_market_is_enabled? && pundit_allow(Family, :can_view_username_and_email?) ? 'ajax' : 'disabled'],
          ...
          if individual_market_is_enabled?
            dropdown += [
                ...
                ['Transition Family Members', transition_family_members_insured_families_path(family: row.id, family_actions_id: "family_actions_#{row.id}"), transition_family_members_link_type(row, pundit_allow(Family, :can_transition_family_members?)) ? 'ajax' : 'disabled']
