...
module Effective
  module Datatables
    class QuoteDatatable < Effective::MongoidDatatable
      ...
      datatable do
        table_column :employer_name, proc: proc { |row|
          row.employer_profile.present? ? row.employer_profile.legal_name : row.employer_name
        }, :sortable => false, :filter => false
        table_column :employer_type, proc: proc { |row| row.employer_type }, label: 'Employer Type', :sortable => false, :filter => false
        table_column :quote, proc: proc { |row|
                             ...
                             :sortable => false, :filter => false
        table_column :effective_date, :proc => proc { |row| row.start_on }, :filter => false
        table_column :claim_code, proc: proc { |row| row.claim_code}, :sortable => false, :filter => false
        table_column :family_count, proc: proc { |row| row.quote_households.count}, :sortable => false, :filter => false
        table_column :state, proc: proc { |row| row.aasm_state}, :sortable => false, :filter => false
        table_column :actions, :width => '50px', :proc => proc { |row|
                                                            ...
                                                            render partial: 'datatables/shared/dropdown', locals: {dropdowns: dropdown, row_actions_id: "quote_actions_#{row.id}"}, formats: :html
