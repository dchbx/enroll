      ...
      datatable do
        ...
        :proc => Proc.new { |row| 
          content_tag(:span, class: 'name') do
            row.census_employee.full_name.try(:titleize)
          end +
          content_tag(:span, 
            content_tag(:p, "DOB: #{format_date row.census_employee.dob}")
          ) +
          content_tag(:span, 
            content_tag(:p, "SSN: #{number_to_obscured_ssn row.census_employee.ssn}")
          ) +
          content_tag(:span, 
             "HIRED:  #{format_date row.census_employee.hired_on}"
          )
