...
class Admin::Aptc < ApplicationController
  ...
  class << self
    ...
    def build_household_level_aptc_csr_data(year, family, hbxs = nil, max_aptc = nil, csr_percentage = nil, applied_aptc_array = nil,  member_ids = nil)
      ...
      { "max_aptc" => max_aptc_vals, "available_aptc" => avalaible_aptc_vals, "csr_percentage" => csr_percentage_vals}
    end

    def build_avalaible_aptc_values(year, family, hbxs, applied_aptc_array = nil, max_aptc = nil,  _member_ids = nil)
      available_aptc_hash = {}
      #max_aptc_vals             = build_max_aptc_values(family, max_aptc)
      max_aptc_vals = build_max_aptc_values(year, family, max_aptc, hbxs)
      total_aptc_applied_vals_for_household = Hash[$months_array.map { |x| [x, format('%.2f', 0.0)] }] # Initialize a Hash for monthly values.
      hbxs.each do |hbx|
        aptc_applied_vals_for_enrollment = build_aptc_applied_values_for_enrollment(year, family, hbx, applied_aptc_array)
        total_aptc_applied_vals_for_household = total_aptc_applied_vals_for_household.merge(aptc_applied_vals_for_enrollment) { |_k, a_value, b_value| a_value.to_f + b_value.to_f } # Adding values of two similar hashes.
      end
      #subtract each value of aptc_applied hash from the max_aptc hash to get APTC Available.
      max_aptc_vals.merge(total_aptc_applied_vals_for_household) { |_k, a_value, b_value| format('%.2f', (a_value.to_f >= b_value.to_f ? (a_value.to_f - b_value.to_f) : a_value.to_f - b_value.to_f)) }
    end
    ...
    def build_household_members(year, family, max_aptc = nil)
      individuals_covered_array = []
      max_aptc = max_aptc.present? ? max_aptc.to_f : (begin
                                                        family.active_household.latest_active_tax_household_with_year(year).latest_eligibility_determination.max_aptc.to_f
                                                      rescue StandardError
                                                        0
                                                      end)
      ratio_by_member = family.active_household.latest_active_tax_household_with_year(year).try(:aptc_ratio_by_member)
      family.family_members.each_with_index do |one_member, _index|

        individuals_covered_array << {one_member.person.id.to_s => [ratio_by_member[one_member.id.to_s] * max_aptc, max_aptc]}
      rescue StandardError
        nil
         # Individuals and their assigned APTC Ratio
      end
      individuals_covered_array
    end

    def build_enrollments_data(year, family, hbxs, applied_aptc_array = nil, max_aptc = nil, csr_percentage = nil, _member_ids = nil)
      enrollments_data = {}
      hbxs.each do |hbx|
    ...
    def build_enrollment_level_aptc_csr_data(year, family, hbx, applied_aptc_array = nil, _max_aptc = nil, _csr_percentage = nil,  _member_ids = nil) #TODO: Last param remove
      ...
      { "aptc_applied" => aptc_applied_vals, "aptc_applied_per_member" => aptc_applied_per_member_vals }
    end
    ...
    def build_aptc_applied_values_for_enrollment(year, family, current_hbx, applied_aptc_array = nil)
      ...
      aptc_applied_hash = {}
      $months_array.each_with_index do |month, _ind|
    ...
    def update_aptc_applied_hash_for_month(year, aptc_applied_hash, current_hbx, month, hbx_iter, _family, applied_aptc_array = nil)
      ...
      if first_of_month_num_current_year >= hbx_iter.effective_on.to_date
        ...
        aptc_applied_hash.store(month, format('%.2f', 0.00)) if aptc_applied_hash[month].blank? #dont mess with the past values
      end
    end

    def build_aptc_applied_per_member_values_for_enrollment(family, current_hbx, aptc_applied_vals, _applied_aptc_array = nil)
      ...
      current_hbx.hbx_enrollment_members.each do |hem|
        ...
        aptc_applied_member_vals = aptc_applied_vals.inject({}) { |h, (k, v)| h[k] = format('%.2f', (v.to_f * ratio_for_this_member.to_f / percent_sum.to_f)); h }
    ...
    def update_max_aptc_hash_for_month(max_aptc_hash, year, month, ed, max_aptc = nil, hbxs = nil)
      ...
      if max_aptc.present?
        ...
        if effective_starting_on > TimeKeeper.date_of_record
          ...
          max_aptc_value = first_of_month_num_current_year >= TimeKeeper.datetime_of_record ? max_aptc : ed.max_aptc.to_f  if hbxs.blank?
          if hbxs.present?
            max_aptc_value = first_of_month_num_current_year >= find_enrollment_effective_on_date(TimeKeeper.datetime_of_record).to_date ? max_aptc : ed.max_aptc.to_f
          end # Incase there are active enrollments, follow 15th of the month rule.
        end
      else
      ...
      if first_of_month_num_current_year >= ed.determined_at.to_date
        ...
        max_aptc_hash.store(month, format('%.2f', 0)) if max_aptc_hash[month].blank?
      end
    end
    ...
    def update_csr_percentages_hash_for_month(csr_percentage_hash, year, month, ed, csr_percentage = nil)
      ...
      csr_percentage_value = if csr_percentage.present?
        # this is when we check available aptc. We only want to update the current and future fields with the updated value.
                               if first_of_month_num_current_year >= TimeKeeper.datetime_of_record
                                 ...
                                 ed.csr_percent_as_integer
                                                      end
                             else
                               ed.csr_percent_as_integer == -1 ? "limited" : ed.csr_percent_as_integer
                             end
      # Check if  'month' >= EligibilityDetermination.determined_at date?
      if first_of_month_num_current_year >= ed.determined_at.to_date
    ...
    def calculate_slcsp_value(_year, family, member_ids = nil)
    ...
    def build_eligible_members(family, member_ids = nil)
      ...
      tax_household_members.each do |member|
        eligible_members << member.person.id.to_s if member.is_ia_eligible
      end
      eligible_members
    end
    ...
    def redetermine_eligibility_with_updated_values(family, params, hbxs, year)
      ...
      unless (params[:max_aptc].to_f == max_aptc.to_f) && (csr_percentage_param == csr_percent_as_integer) # If any changes made to MAX APTC or CSR
        ...
        eligibility_date = if effective_starting_on > TimeKeeper.date_of_record
                             effective_starting_on
                           else
                             hbxs.present? ? find_enrollment_effective_on_date(TimeKeeper.datetime_of_record) : TimeKeeper.datetime_of_record # Follow 15th of month rule if active enrollment.
                           end
    ...
    def update_aptc_applied_for_enrollments(family, params, year)
      ...
      params.each do |key, aptc_value|
        next unless key.include?('aptc_applied_')
        ...
        next unless actual_aptc_value != updated_aptc_value # TODO: check if the effective_on doesnt go to next year?????
        ...
        duplicate_hbx.hbx_enrollment_members = original_hbx.hbx_enrollment_members.collect(&:dup)
    ...
    def build_error_messages(max_aptc, _csr_percentage, applied_aptcs_array, year, hbxs)
      sum_of_all_applied = 0.0
      aptc_errors = {}
      aptc_errors["EFFECTIVE_DATE_OVERFLOW"] = Settings.aptc_errors.effective_date_overflow if hbxs.present? && find_enrollment_effective_on_date(TimeKeeper.datetime_of_record).year != year
      ...
      if applied_aptcs_array.present?
        applied_aptcs_array.each do |hbx|
          ...
          if applied_aptc > max_for_hbx
            aptc_errors["ENROLLMENT_MAX_SMALLER_THAN_APPLIED"] = Settings.aptc_errors.enrollment_max_smaller_than_applied + "[NEW_MAX_FOR_ENROLLMENT (#{'%.2f' % max_for_hbx.to_s}) < APPLIED_APTC (#{'%.2f' % applied_aptc.to_s})] "
          end
    ...
    def years_with_tax_household(family)
      ...
      year_set << TimeKeeper.date_of_record.next_year.year if current_hbx&.under_open_enrollment? && oe_start_year == current_year
