...
module Admin
  class Aptc < ApplicationController
  ...
  #TODO: Last param remove
      def build_enrollment_level_aptc_csr_data(year, family, hbx, applied_aptc_array = nil, _max_aptc = nil, _csr_percentage = nil,  _member_ids = nil)
      ...
      def update_aptc_applied_hash_for_month(year, aptc_applied_hash, current_hbx, month, hbx_iter, _family, applied_aptc_array = nil)
        ...
        elsif aptc_applied_hash[month].blank?
          aptc_applied_hash.store(month, format('%.2f', 0.00))
        end
      end
      ...
      def update_max_aptc_hash_for_month(max_aptc_hash, year, month, ed, max_aptc = nil, hbxs = nil)
        ...
        elsif max_aptc_hash[month].blank?
          max_aptc_hash.store(month, format('%.2f', 0))
        end
      ...
      def update_csr_percentages_hash_for_month(csr_percentage_hash, year, month, ed, csr_percentage = nil)
        ...
        elsif csr_percentage_hash[month].blank?
          csr_percentage_hash.store(month, 0)
        end
      ...
      def last_of_month_converter(month, year = TimeKeeper.date_of_record.year)
        ...
        Date.parse("#{year}-#{month_num}-#{last_day}")
      end
      ...
      def update_aptc_applied_for_enrollments(family, params, year)
        ...
        params.each do |key, aptc_value|
          ...
          elsif original_hbx.may_cancel_coverage?
        ...
        if applied_aptcs_array.present?
          applied_aptcs_array.each do |hbx|
            ...
            if applied_aptc > max_for_hbx
              aptc_errors["ENROLLMENT_MAX_SMALLER_THAN_APPLIED"] =
                Settings.aptc_errors.enrollment_max_smaller_than_applied + "[NEW_MAX_FOR_ENROLLMENT (#{'%.2f' % max_for_hbx.to_s}) < APPLIED_APTC (#{'%.2f' % applied_aptc.to_s})] "
            end
