class Household
  ...
  def build_or_update_tax_households_and_eligibility_determinations(verified_family, primary_person, active_verified_household, new_dependents)
    ...
    if tax_households.present?
      tax_households.where(effective_ending_on: nil).each do |thh|
    ...
    verified_tax_households.each do |verified_tax_household|
      next unless verified_tax_household.present? && verified_tax_household.eligibility_determinations.present?
  ...
  def build_tax_household_member(tax_household, tax_household_member, new_dependents)
    family_member = nil
    ...
    tax_household.tax_household_members.build(
      ...
      is_subscriber: true,
      is_ia_eligible: tax_household_member.is_insurance_assistance_eligible || false,
  ...
  def build_verified_determinations(tax_household, verified_tax_household)
    ...
    verified_tax_households.each do |vthh|
      ...
      if tax_households_hbx_assigned_ids.include?(vthh.hbx_assigned_id)
        ...
        vthh.tax_household_members.each do |thhm|
          ...
          update_verified_applicants(application_in_context, verified_family, thhm)
        end
        #Applicant/TaxHouseholdMember block end
        #Eligibility determination start.
        build_household_determinations(vthh, tax_household, benchmark_plan_id) unless verified_tax_households.map(&:eligibility_determinations).map(&:present?).include?(false)
