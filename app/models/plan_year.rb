...
class PlanYear
  ...
  PUBLISHED = %w[published enrolling enrolled active suspended].freeze
  RENEWING  = %w[renewing_draft renewing_published renewing_enrolling renewing_enrolled renewing_publish_pending].freeze
  RENEWING_PUBLISHED_STATE = %w[renewing_published renewing_enrolling renewing_enrolled].freeze
  DRAFT_STATES = %w[draft renewing_draft].freeze

  INELIGIBLE_FOR_EXPORT_STATES = %w[draft publish_pending eligibility_review published_invalid canceled renewing_draft suspended application_ineligible renewing_application_ineligible renewing_canceled conversion_expired].freeze

  OPEN_ENROLLMENT_STATE   = %w[enrolling renewing_enrolling].freeze
  INITIAL_ENROLLING_STATE = %w[publish_pending eligibility_review published published_invalid enrolling enrolled].freeze
  INITIAL_ELIGIBLE_STATE  = %w[published enrolling enrolled].freeze
  ...
  scope :published_plan_years_within_date_range, lambda { |begin_on, end_on|
    where(
      "$and" => [
  ...
  scope :published_plan_years_by_date, lambda { |date|
    where(
      "$and" => [
        {:aasm_state.in => PUBLISHED },
  ...
  scope :published_and_expired_plan_years_by_date, lambda { |date|
    where(
      "$and" => [
  ...
  def ensure_benefit_group_is_valid
    self.benefit_groups.each do |bg|
      next unless bg.sole_source?
      bg.build_composite_tier_contributions if bg.composite_tier_contributions.empty?
  ...
  def eligible_for_export?
    ...
    return(false) if start_on.blank?
    return false if INELIGIBLE_FOR_EXPORT_STATES.include?(self.aasm_state.to_s)
    if TimeKeeper.date_of_record < start_on
      if enrolled?
        return true if open_enrollment_completed? && binder_paid? && past_transmission_threshold?
      elsif renewing_enrolled?
        return true if open_enrollment_completed? && past_transmission_threshold?
      end
      return false
    end
    true
  ...
  def hbx_enrollments
    @hbx_enrollments = [] if benefit_groups.empty?
  ...
  def is_offering_dental?
    benefit_groups.any?(&:is_offering_dental?)
  end
  ...
  def application_errors
    ...
    log_message(errors){{benefit_groups: "Reference plans have not been selected for benefit groups. Please edit the plan year and select reference plans."}} if benefit_groups.any?{|bg| bg.reference_plan_id.blank? }

    log_message(errors) {{benefit_groups: "You must create at least one benefit group to publish a plan year"}} if benefit_groups.blank?

    log_message(errors) {{benefit_groups: "Every employee must be assigned to a benefit group defined for the published plan year"}} if employer_profile.census_employees.active.to_set != assigned_census_employees.to_set

    log_message(errors) {{employer_profile:  "This employer is ineligible to enroll for coverage at this time"}} if employer_profile.ineligible?

    log_message(errors) {{ publish: "You may only have one published plan year at a time" }} if overlapping_published_plan_year?
  ...
  def application_eligibility_warnings
    ...
    warnings.merge!({primary_office_location: "Is a small business located in #{Settings.aca.state_name}"}) unless employer_profile.is_primary_office_local?
    ...
    warnings.merge!({ fte_count: "Has 1 -#{Settings.aca.shop_market.small_market_employee_count_maximum} full time equivalent employees" }) if fte_count < 1 || fte_count > Settings.aca.shop_market.small_market_employee_count_maximum
    ...
    unless effective_date.yday == 1
      ...
      if !benefit_groups.empty? && (minimum_employer_contribution < Settings.aca.shop_market.employer_contribution_percent_minimum)
  ...
  def enrollment_errors
    ...
    errors.merge!(eligible_to_enroll_count: "at least one employee must be eligible to enroll") if eligible_to_enroll_count == 0
    ...
    if non_business_owner_enrolled.count < eligible_to_enroll_count
      errors.merge!(non_business_owner_enrollment_count: "at least #{non_owner_participation_count_minimum} non-owner employee must enroll") if non_business_owner_enrolled.count < non_owner_participation_count_minimum
    end
  ...
  def application_warnings
    application_eligibility_warnings.each_pair{ |_key, value| self.errors.add(:base, value) } unless is_application_eligible?
  end

  class << self
    def find(id)
      organizations = Organization.where("employer_profile.plan_years._id" => BSON::ObjectId.from_string(id))
      !organizations.empty? ? organizations.first.employer_profile.plan_years.unscoped.detect { |py| py._id.to_s == id.to_s} : nil
    end
    ...
    def map_binder_payment_due_date_by_start_on(start_on)
      ...
      event_arr = [{event_name: "New Year's Day", event_date: schedule_time(Date.new(Date.today.year, 0o1, 0o1))}, {event_name: "Martin birthday", event_date: nth_wday(3, 1, 1, Date.today.year)}, {event_name: "President's Day", event_date: nth_wday(3, 1, 2, Date.today.year)}, {event_name: "Memorial Day", event_date: last_monday_may(Date.today.year, 5, 31)}, {event_name: "Labor day", event_date: nth_wday(1, 1, 9, Date.today.year)}, {event_name: "Columbus Day", event_date: nth_wday(2, 1, 10, Date.today.year)}, {event_name: "Veterans Day", event_date: schedule_time(Date.new(Date.today.year, 11, 11))}, {event_name: "Thanksgiving Day", event_date: nth_wday(4, 4, 11, Date.today.year)}, {event_name: "Christmas Day", event_date: schedule_time(Date.new(Date.today.year, 12, 25))}, {event_name: "Independence Day", event_date: schedule_time(Date.new(Date.today.year, 0o7, 0o4))}]
      ...
      while event_date_arr.include?(to_date) || (to_date.wday == 6) || (to_date.wday == 0)
        to_date += 1.day #If to_date is in holidays arr, we are adding +1 day
      end
  ...
  def adjust_open_enrollment_date
    update_attributes(:open_enrollment_start_on => TimeKeeper.date_of_record) if TimeKeeper.date_of_record > open_enrollment_start_on && TimeKeeper.date_of_record < open_enrollment_end_on
  end
  ...
  def schedule_employee_terminations(terminated_on = TimeKeeper.date_of_record.end_of_month)
    hbx_enrollments.each do |hbx_enrollment|
      hbx_enrollment.schedule_coverage_termination!(terminated_on) if hbx_enrollment.may_schedule_coverage_termination?
    end
  end
  ...
  def deny_enrollment
    employer_profile.enrollment_denied! if employer_profile.may_enrollment_denied?
  end

  def is_eligible_to_match_census_employees?
    !benefit_groups.empty? &&
      (published? || enrolling? || enrolled? || active?)
  end
  ...
  def link_census_employees
    self.employer_profile.census_employees.eligible_without_term_pending.each(&:save)
  end

  alias external_plan_year? can_be_migrated?
  ...
  def record_sic_and_rating_area
    ...
    self.benefit_groups.each(&:estimate_composite_rates)
  end

  def trigger_renewal_notice
    return true if benefit_groups.any?(&:is_congress?)
    event_name = aasm.current_event.to_s.gsub(/!/, '')
  ...
  def renewal_group_notice
    event_name = aasm.current_event.to_s.gsub(/!/, '')
    return true if benefit_groups.any?(&:is_congress?) || ["publish","withdraw_pending","revert_renewal"].include?(event_name)
  ...
  def finalize_composite_rates
    benefit_groups.each(&:finalize_composite_rates)
  end

  def renewal_successful
    employer_profile.transmit_renewal_eligible_event if transmit_employers_immediately?
  end
  ...
  def open_enrollment_date_checks
    ...
    errors.add(:start_on, "must be first day of the month") if start_on != start_on.beginning_of_month

    errors.add(:end_on, "benefit period may not exceed #{Settings.aca.shop_market.benefit_period.length_maximum.year} year") if end_on > start_on.years_since(Settings.aca.shop_market.benefit_period.length_maximum.year)

    errors.add(:start_on, "can't occur before open enrollment end date") if open_enrollment_end_on > start_on

    errors.add(:open_enrollment_end_on, "can't occur before open enrollment start date") if open_enrollment_end_on < open_enrollment_start_on

    errors.add(:open_enrollment_start_on, "can't occur before 60 days before start date") if open_enrollment_start_on < (start_on - Settings.aca.shop_market.open_enrollment.maximum_length.months.months)
    ...
    unless ['canceled', 'suspended', 'terminated','termination_pending'].include?(aasm_state)
