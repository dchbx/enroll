...
module Factories
  class TempPlanYearRenewalFactory
    ...
    def renew
      ...
      begin
        ...
        @active_plan_year = @employer_profile.published_plan_year
        ...
        if @renewal_plan_year.may_renew_plan_year?
          ...
          raise TempPlanYearRenewalFactoryError,
        ...
        if @renewal_plan_year.save
          ...
          @renewal_plan_year.force_publish! # requested by business.
          @renewal_plan_year
        else
          raise TempPlanYearRenewalFactoryError,
                "For employer: #{@employer_profile.inspect}, \n" \
    ...
    def validate_employer_profile
      raise TempPlanYearRenewalFactoryError, "Employer #{@employer_profile.legal_name} already renewed" if @employer_profile.plan_years.renewing.any?

      raise TempPlanYearRenewalFactoryError, "Renewals require an existing, published Plan Year" unless PlanYear::PUBLISHED.include? @employer_profile.published_plan_year.aasm_state

      raise TempPlanYearRenewalFactoryError, "Employer primary address must be located in #{Settings.aca.state_name}" unless @employer_profile.is_primary_office_local?
    end
    ...
    def renew_benefit_groups
      @active_plan_year.benefit_groups.each do |active_group|
        ...
        if new_group.save
          ...
          raise TempPlanYearRenewalFactoryError, message
        end
      end
    end
    ...
    def clone_benefit_group(active_group)
      ...
      raise TempPlanYearRenewalFactoryError, "Unable to find renewal for referenence plan: Id #{active_group.reference_plan.id} Year #{active_group.reference_plan.active_year} Hios #{active_group.reference_plan.hios_id}" if reference_plan_id.blank?

      elected_plan_ids = reference_plan_ids(active_group)
      raise TempPlanYearRenewalFactoryError, "Unable to find renewal for elected plans: #{active_group.elected_plan_ids}" if elected_plan_ids.blank?
    ...
    def renew_census_employees(active_group, new_group)
      eligible_employees(active_group).each do |census_employee|
        ...
        raise TempPlanYearRenewalFactoryError, "For employer: #{@employer_profile.inspect}, unable to save census_employee: #{census_employee.inspect}" unless census_employee.renewal_benefit_group_assignment.save
      end
      true
    end
  ...
  class TempPlanYearRenewalFactoryError < StandardError; end
end
