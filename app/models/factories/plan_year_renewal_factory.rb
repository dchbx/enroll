...
module Factories
  class PlanYearRenewalFactory
    ...
    def validate_employer_profile
      raise PlanYearRenewalFactoryError, "Employer #{@employer_profile.legal_name} already renewed" if @employer_profile.plan_years.renewing.any?

      raise PlanYearRenewalFactoryError, "Renewals require an existing, published Plan Year" unless PlanYear::PUBLISHED.include? @active_plan_year.aasm_state

      raise PlanYearRenewalFactoryError, "Renewal time period has expired.  You must submit a new application" unless TimeKeeper.date_of_record <= @active_plan_year.end_on
    end

    def validate_plan_year
      raise PlanYearRenewalFactoryError, "Employer #{@employer_profile.legal_name} don't have active application for renewal" if @active_plan_year.blank?

      @active_plan_year.benefit_groups.each do |benefit_group|
        reference_plan_id = benefit_group.reference_plan.renewal_plan_id
        raise PlanYearRenewalFactoryError, "Unable to find renewal for referenence plan: Id #{benefit_group.reference_plan.id} Year #{benefit_group.reference_plan.active_year} Hios #{benefit_group.reference_plan.hios_id}" if reference_plan_id.blank?

        elected_plan_ids = benefit_group.renewal_elected_plan_ids
        raise PlanYearRenewalFactoryError, "Unable to find renewal for elected plans: #{benefit_group.elected_plan_ids}" if elected_plan_ids.blank?
      end
    end
    ...
    def renewal_elected_plan_ids(active_group)
      ...
      case active_group.plan_option_kind
      when "single_carrier"
    ...
    def clone_benefit_group(active_group)
      ...
      raise PlanYearRenewalFactoryError, "Unable to find renewal for referenence plan: Id #{active_group.reference_plan.id} Year #{active_group.reference_plan.active_year} Hios #{active_group.reference_plan.hios_id}" if renewal_plan.blank?
      ...
      if active_group.sole_source?
        ...
        raise PlanYearRenewalFactoryError, "Unable to renew. renewal plan for referenence plan: #{active_group.reference_plan.id} not offered in employer service areas" unless service_area_hios.include?(renewal_plan.hios_id)
      end

      elected_plan_ids = renewal_elected_plan_ids(active_group)
      raise PlanYearRenewalFactoryError, "Unable to find renewal for elected plans: #{active_group.elected_plan_ids}" if elected_plan_ids.blank?
    ...
    def renew_census_employees(active_group, new_group)
      eligible_employees(active_group).each do |census_employee|
        next unless census_employee.active_benefit_group_assignment #&& census_employee.active_benefit_group_assignment.benefit_group_id == active_group.id
        census_employee.add_renew_benefit_group_assignment([new_group])

        raise PlanYearRenewalFactoryError, "For employer: #{@employer_profile.inspect}, unable to save census_employee: #{census_employee.inspect}" unless census_employee.renewal_benefit_group_assignment.save
      end
      true
    end
    ...
    def generate_employee_role_notices; end

    def generate_employer_profile_notices; end
  end
