    ...
    def self.load_record_cache!
      ...
      Plan.all.where(:active_year.gte => TimeKeeper.date_of_record.year).each do |plan|
        ...
        plan.premium_tables.each do |pt|
          $plan_age_lookup[plan.id][pt.age] = [] unless $plan_age_lookup[plan.id].key?(pt.age)
          ...
          $plan_age_lookup_with_rating_area[plan.id][pt.age] = {} unless $plan_age_lookup_with_rating_area[plan.id].key?(pt.age)
          $plan_age_lookup_with_rating_area[plan.id][pt.age][pt.rating_area] = [] unless $plan_age_lookup_with_rating_area[plan.id][pt.age].key?(pt.rating_area)
