...
#managing profile staff
  ...
  def billing_plan_year(billing_date = nil)
    ...
    if billing_date.blank?
      if plan_year.blank? && plan_year = (plan_years.published + plan_years.renewing_published_state).detect{|py| py.start_on > billing_report_date && py.open_enrollment_contains?(TimeKeeper.date_of_record) }
        billing_report_date = plan_year.start_on
      end

      if plan_year.blank? && plan_year = find_plan_year_by_effective_date(TimeKeeper.date_of_record)
        billing_report_date = TimeKeeper.date_of_record
      end

      if plan_year.blank? && plan_year = (plan_years.published + plan_years.renewing_published_state).detect{|py| py.start_on > billing_report_date }
        billing_report_date = plan_year.start_on
      end
    end
      ...
      if plan_year.save!
        ...
        if import_census_employee == true
          quote.quote_households.each do |qhh|
            ...
            ce.create_benefit_group_assignment(plan_year.benefit_groups.find(benefit_group_mapping[qhh.quote_benefit_group_id.to_s].to_s).to_a)
  ...
  class << self
    ...
    def find(id)
      ...
      organizations.empty? ? nil : organizations.first.employer_profile
...
#DC Publish plan year reminder notice
        ...
        transmit_scheduled_employers(new_date) if aca_shop_market_transmit_scheduled_employers && ((new_date.prev_day.mday + 1) == aca_shop_market_employer_transmission_day_of_month)
...
# ability to create library of templates for employer profiles
  ...
  def notify_broker_added
    ...
    notify("acapi.info.events.employer.broker_added", {employer_id: self.hbx_id, event_name: "broker_added"}) if changed_fields.present? && changed_fields.include?("start_on")
  end
  ...
  def terminate(termination_date)
    ...
    if plan_year.present?
      if termination_date >= plan_year.start_on
        ...
        plan_year.terminate! if termination_date < TimeKeeper.date_of_record && plan_year.may_terminate?
      elsif plan_year.may_cancel?
        plan_year.cancel!
      end
