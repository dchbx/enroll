...
module ApplicationHelper
  ...
  def shopping_group_premium(plan_cost, plan_ehb_cost, can_use_aptc = true) # rubocop:disable Style/OptionalBooleanParameter
...
# the following methods are used when we are embedding devise signin and signup pages in views other than devise.
  ...
  def is_readonly(object)
    ...
    !(object.new_record? || object.try(:eligible?)) # employer census roster
  end
  ...
  def bulk_notice_id_discovery(id)
    BenefitSponsors::Organizations::Organization.where(_id: id).first&.hbx_id ||
      Person.where(_id: id).first&.hbx_id
  end
end
