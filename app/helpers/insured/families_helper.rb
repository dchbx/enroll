  ...
  module FamiliesHelper
    ...
    def plan_shopping_dependent_text(hbx_enrollment)
      ...
      if subscriber.present? && dependents.count == 0
        ...
        link_to(pluralize(dependents.count, "dependent"), "", data: {toggle: "modal", target: "#dependentsList"},
                                                              class: "dependent-text").html_safe + render(partial: "shared/dependents_list_modal", locals: {subscriber: subscriber, dependents: dependents})
      else
        ("<span class='dependent-text'>#{subscriber.first.person.full_name}</span>" + " + " + link_to(pluralize(dependents.count, "dependent"), "", data: {toggle: "modal", target: "#dependentsList"},
                                                                                                                                                    class: "dependent-text")).html_safe + render(partial: "shared/dependents_list_modal",
                                                                                                                                                                                                 locals: {subscriber: subscriber, dependents: dependents})
      end
    end
