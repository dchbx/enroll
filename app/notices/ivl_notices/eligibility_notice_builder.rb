...
class IvlNotices::EligibilityNoticeBuilder < IvlNotice
  ...
  def append_data
    ...
    latest_application = family.applications.where(:aasm_state.nin => ["draft"]).max_by(&:submitted_at)
  ...
  def append_applicant_information(applicant)
    ...
    reason_for_ineligibility << "this person isn’t a resident of the District of Columbia. Go to healthcare.gov to learn how to apply for coverage in the right state." unless applicant.person.is_dc_resident?
    ...
    if lawful_presence_outstanding?(applicant.person)
    ...
    PdfTemplates::Individual.new({
                                   ...
                                   no_aptc_because_of_income: applicant.preferred_eligibility_determination.aptc_csr_annual_household_income > applicant.preferred_eligibility_determination.aptc_annual_income_limit,
                                   no_csr_because_of_income: applicant.preferred_eligibility_determination.aptc_csr_annual_household_income > applicant.preferred_eligibility_determination.csr_annual_income_limit,
