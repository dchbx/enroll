  ...
  module Individual
    ...
    class ApplyAggregateToEnrollment
      ...
      def fetch_enrollments_to_renew(eligibility_determination)
        ...
        enrollment_list = enrollments.reject{|enr| enr.product.metal_level_kind == :catastrophic}
      ...
      def generate_enrollments(enrollments, eligibility_determination)
        ...
        enrollments.each do |enrollment|
          max_aptc = if EnrollRegistry[:calculate_monthly_aggregate].feature.is_enabled
                       max_aptc = EnrollRegistry[:calculate_monthly_aggregate] { {hbx_enrollment: enrollment} }
                       max_aptc.success? ? max_aptc.value! : 0
                     else
                       current_max_aptc
                     end
