...
class RemoveCoverageHouseholdMember < MongoidMigrationTask
  def migrate
    ...
    coverage_household_members = if action == "remove_invalid_fm"
                                   []
                                 else
                                   ch.coverage_household_members.where(family_member_id: family_member.id)
                                 end
    if person.blank?
      ...
      ch.coverage_household_members.where(family_member_id: family_member || ENV['family_member_id'].to_s).each(&:destroy)
      ch.save
