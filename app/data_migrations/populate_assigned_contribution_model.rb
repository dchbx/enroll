    ...
    while offset <= benefit_sponsorships.count
      benefit_sponsorships.offset(offset).limit(batch_size).no_timeout.each do |benefit_sponsorship|
        ...
        benefit_sponsor_catalog.product_packages.where(product_kind: :health).each do |product_package|
          ...
          next unless contribution_unit.minimum_contribution_factor == 0
          ...
          unless Rails.env.test?
            puts "#{benefit_sponsorship.legal_name} - #{benefit_sponsor_catalog.created_at} - #{benefit_application.created_at} - #{benefit_application.start_on} - #{contribution_unit.minimum_contribution_factor} - #{contribution_unit.default_contribution_factor}"
          end
        end
