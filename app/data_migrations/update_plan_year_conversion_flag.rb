...
class UpdatePlanYearConversionFlag < MongoidMigrationTask
  ...
  def migrate
    ...
    feins.each do |fein|
      ...
      next unless plan_years && employer_profile.is_conversion?
      ...
      external_plan_years += plan_years.select do |py|
      ...
      puts format("updated conversion flag on employer: %s  using fein %s", organization.legal_name, fein)
    end
  rescue Exception => e
    puts e.message
  end
