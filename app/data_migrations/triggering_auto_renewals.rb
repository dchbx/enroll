  ...
  def migrate
    ...
    Organization.where(:"employer_profile.plan_years" => {:$elemMatch => {:start_on => plan_year_start_on, :aasm_state => 'renewing_enrolling'}}, :'employer_profile.profile_source' => 'conversion').each do |org|
      org.employer_profile.census_employees.each do |ce|
        ...
        next unless enrollments.present? && enrollments.where(aasm_state: "renewing_waived").present?
        count += 1
    ...
    unless Rails.env.test?
      puts "The total effected ER's count is #{Organization.where(:"employer_profile.plan_years" => {:$elemMatch => {:start_on => plan_year_start_on, :aasm_state => 'renewing_enrolling'}},
                                                                  :'employer_profile.profile_source' => 'conversion').count}"
    end
