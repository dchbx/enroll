...
class AddNewEligibilityDetermination < MongoidMigrationTask
  def migrate
    ...
    people.each do |person|
      ...
      latest_active_tax_household = if active_household.latest_active_tax_household_with_year(date.year).nil?
  ...
  def get_people
    ...
    hbx_ids.inject([]) do |people, hbx_id|
      if Person.where(hbx_id: hbx_id).size == 1
        people << Person.where(hbx_id: hbx_id).first #unless Rails.env.test?
      else
        puts "No person was found with the given hbx_id #{hbx_id}"
      end
    end
  end
end
