        ...
        if enrollments.count > 1
          ...
          if selected_enrollments.size == 1
            ...
            renewal_enrollments.each do |renewal_enrollment|
              renewal_correct_enrollments << renewal_enrollment if renewal_enrollment.updated_at.strftime("%m/%d/%Y %I:%M%p") == selected_enrollments.first.updated_at.strftime("%m/%d/%Y %I:%M%p")
            end
            if renewal_correct_enrollments.size == 1
              ...
              renewal_enrollment.update_attribute(:aasm_state, "coverage_enrolled") if hbx_enrollment.effective_on == renewal_enrollment.effective_on && hbx_enrollment.coverage_kind != renewal_enrollment.coverage_kind
            end
          end
        end
      end
