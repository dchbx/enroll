...
class FixPlanYearsToObeyMinimumOpenEnrollmentPeriod < MongoidMigrationTask
  ...
  def migrate
    ...
    oe_minimum_validation_error_message = "open enrollment period is less than minimum: 5 days"
    py_update_count = 0
    plan_years.each do |py|
      minimum_length = if PlanYear::RENEWING.include?(py.aasm_state)
                         Settings.aca.shop_market.renewal_application.open_enrollment.minimum_length.days
                       else
                         Settings.aca.shop_market.open_enrollment.minimum_length.days
                       end

      next unless py.open_enrollment_date_errors.values.flatten.include?("Open Enrollment period is shorter than minimum (#{minimum_length} days)")
